...
namespace Cofoundry.Domain.Tests.Integration.Users.Commands
{
    ...
    public class CompleteUserAccountRecoveryCommandHandlerTests
    {
        const string UNIQUE_PREFIX = "CompPWResetCHT ";

        private readonly DbDependentTestApplicationFactory _appFactory;

        public CompleteUserAccountRecoveryCommandHandlerTests(
        ...
        public async Task InvalidatesOldResetTokens()
        {
            ...
            var resetRequest1Result = resetRequests.SingleOrDefault(r => r.UserAccountRecoveryRequestId == resetRequest1.UserAccountRecoveryRequestId);
            var resetRequest2Result = resetRequests.SingleOrDefault(r => r.UserAccountRecoveryRequestId == resetRequest2.UserAccountRecoveryRequestId);
            var resetRequest3Result = resetRequests.SingleOrDefault(r => r.UserId == resetRequest1.UserId && r.UserAccountRecoveryRequestId != resetRequest1.UserAccountRecoveryRequestId);
        ...
        private static async Task<UserAccountRecoveryRequest> GetResetRequest(
            DbDependentTestApplication app,
            int userId
            )
        {
