This library allows you to send mail using the cross platform [MailKit](https://github.com/jstedfast/MailKit) library. 

## Installation

Install the [Cofoundry.Plugins.Mail.MailKit](https://www.nuget.org/packages/Cofoundry.Plugins.Mail.MailKit/) package via Nuget, e.g. via the CLI:

```bash
dotnet add package Cofoundry.Plugins.Mail.MailKit
```

## Configuration

By referencing this package your Cofoundry project will automatically replace the default `IMailDispatchService` implementation with one that uses MailKit. Use the following settings to configure the service:
