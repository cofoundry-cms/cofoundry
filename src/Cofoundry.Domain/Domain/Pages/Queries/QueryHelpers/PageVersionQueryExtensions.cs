...
namespace Cofoundry.Domain
{
    public static class PageVersionQueryExtensions
    {
        ...
        /// </summary>
        public static IQueryable<PageVersion> FilterByWorkFlowStatusQuery(
            this IQueryable<PageVersion> dbQuery,
            ...
            switch (workFlowStatusQuery)
            {
                ...
                case WorkFlowStatusQuery.Latest:
                    dbQuery = dbQuery
                        ...
                        .GroupBy(e => e.PageId, (key, g) => g.OrderByDescending(v => v.WorkFlowStatusId == (int)WorkFlowStatus.Draft).FirstOrDefault());
                    break;
                case WorkFlowStatusQuery.PreferPublished:
                    dbQuery = dbQuery
                        .Where(v => v.WorkFlowStatusId == (int)WorkFlowStatus.Draft || v.WorkFlowStatusId == (int)WorkFlowStatus.Published)
                        .GroupBy(e => e.PageId, (key, g) => g.OrderByDescending(v => v.WorkFlowStatusId == (int)WorkFlowStatus.Published).FirstOrDefault());
                    break;
                case WorkFlowStatusQuery.SpecificVersion:
                    dbQuery = dbQuery.Where(v => v.PageVersionId == versionId);
                    break;
                default:
