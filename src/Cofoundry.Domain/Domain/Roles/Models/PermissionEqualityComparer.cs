...
public class PermissionEqualityComparer : IEqualityComparer<IPermission>
{
    public bool Equals(IPermission? x, IPermission? y)
    {
        ...
        if (x.PermissionType == null || y.PermissionType == null)
        {
            return false;
        }

        if (x is IEntityPermission != y is IEntityPermission)
        {
            return false;
        }
        ...
        if (xDef != null && xDef != GetEntityDefinitionCode(y))
        {
            return false;
        }
    ...
    private static string? GetEntityDefinitionCode(IPermission permission)
    {
