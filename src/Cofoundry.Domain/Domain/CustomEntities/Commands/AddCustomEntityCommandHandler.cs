    ...
    , IPermissionRestrictedCommandHandler<AddCustomEntityCommand>
{
    ...
    private static string NormalizeUrlSlug(AddCustomEntityCommand command, CustomEntityDefinitionSummary definition)
    {
        var value = definition.AutoGenerateUrlSlug ? command.Title : command.UrlSlug;
        return SlugFormatter.ToSlug(value);
    }

    private static void ValidateCommand(AddCustomEntityCommand command, CustomEntityDefinitionSummary definition)
    {
        ...
        )
    {
        ...
        if (!isUnique)
        {
            ...
            if (definition.AutoGenerateUrlSlug)
            {
                ...
                var titleTerm = definition.Terms.GetValueOrDefault(CustomizableCustomEntityTermKeys.Title, "title").ToLower();
                message = $"The {titleTerm} '{command.Title}' must be unique (symbols and spaces are ignored in the uniqueness check)";
                prop = "Title";
            }
            else
            {
                var urlSlugTerm = definition.Terms.GetValueOrDefault(CustomizableCustomEntityTermKeys.UrlSlug, "url slug").ToLower();
                message = $"The {urlSlugTerm} '{urlSlug}' must be unique";
                prop = "UrlSlug";
            }
